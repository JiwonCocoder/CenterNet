...
SPLITS = ['3dop', 'subcnn']
import _init_paths
from ddd_utils import project_to_image, project_to_image_sun2, project_to_image_sun, alpha2rot_y, \
...
from ddd_utils import draw_box_3d, unproject_2d_to_3d, draw_box_3d_sun
from ddd_utils import rot_y2alpha, order_rotation_y_matrix, draw_box_3d_world
from image import get_affine_transform, affine_transform, get_3rd_point, get_dir, crop
import pdb
...
# ann_dir = DATA_PATH + 'labelIndexing_untilted/'
ann_dir = DATA_PATH + 'label_iou04/'
# calib_dir = DATA_PATH + '{}/calib/'  # 오류인듯: 뒤에 .format(SPLIT)이 붙어야 할듯
# calib_dir = DATA_PATH + 'calibIndexing/'  # 수정
calib_dir = DATA_PATH + 'calibIndexing_translation/'
Rtilt_dir = DATA_PATH + 'RtiltIndexing/'
img_dir = DATA_PATH + 'img_backwarping_i_all/'
# splits = ['train', 'val']
#splits = ['testDataIndex']
# splits=['train']
# splits = ['train_untilted_ut', 'val_untilted_ut']
# splits = ['train_mini', 'val_mini']
splits = ['allDataIndex_untilted_iou04']
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        opt_input_h = 480
        if keep_res:
            # print("keep_res is fixed")
            s = np.array([opt_input_w, opt_input_h], dtype=np.int32)  # (448, 448)
        else:
        ...
        if np.random.random() < opt_aug_ddd:
            ...
            c[0] += image.shape[1] * np.clip(np.random.randn() * cf, -2 * cf, 2 * cf)
            c[1] += image.shape[0] * np.clip(np.random.randn() * cf, -2 * cf, 2 * cf)

        trans_input = get_affine_transform(c, s, 0, [opt_input_w, opt_input_h])
        dst_in = cv2.resize(image, dsize=(640, 480), interpolation=cv2.INTER_AREA)

        inp = cv2.warpAffine(image, trans_input,
        ...
        trans_output = get_affine_transform(
            c, s, 0, [opt_output_w,opt_output_h])

        out = cv2.warpAffine(image, trans_output,
                             (opt_output_w, opt_output_h),
                             flags=cv2.INTER_LINEAR)
        dst_out = cv2.resize(image, dsize=(opt_output_w, opt_output_h), interpolation=cv2.INTER_AREA)
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            alpha = float(tmp[12])
            xmin, ymin, xmax, ymax = int(bbox2D[0]), int(bbox2D[1]), int(bbox2D[2]), int(bbox2D[3])
            width_2d = xmax - xmin
            height_2d = ymax - ymin
            bbox = np.array([xmin, ymin, xmax, ymax],
                            dtype=np.float32)
            bbox[:2] = affine_transform(bbox[:2], trans_output)  # (112, 112)에 맞게 BR 점의 위치를 옮겨줌
            bbox[2:] = affine_transform(bbox[2:], trans_output)  # (112, 112)에 맞게 TL 점의 위치를 옮겨줌
            ...
            cv2.line(out, (bbox[0], bbox[1]), (bbox[2], bbox[1]), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            cv2.line(out, (bbox[2], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            cv2.line(out, (bbox[2], bbox[3]), (bbox[0], bbox[3]), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            cv2.line(out, (bbox[0], bbox[3]), (bbox[0], bbox[1]), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            cv2.imshow("out_bbox", out)

            # ct_x = int(bbox2D[0])
            # ct_y = int(bbox2D[1])
            # xmin = int(ct_x - width_2d/2)
            # ymin = int(ct_y - height_2d/2)
            # xmax = int(ct_x + width_2d/2)
            # ymax = int(ct_y + height_2d/2)
            ...
            ct_x = xmin + width_2d / 2
            ct_y = ymin + height_2d / 2
            ...
            location_cam_untilted = [location[i] for i in sun_permutation]
            location_cam_untilted[1] *= -1
            ...
            #un Rtilt 적용된 location을 2d image에 project시킨것.
            #red circle
            ct3d_to_ct2d = project_to_image(np.array([location_cam_untilted]), calib)  # 이제 3d bounding box를 image에 투영시킴
            ...
            # location_changed를 project시킨 값(red circle)이 image 크기 내에 존재할 때
            # json에 추가시키기.
            if(0<=ct3d_to_ct2d[0][0]<width) and (0 <=ct3d_to_ct2d[0][1]< height):
                ...
                bbox2D = [float(bbox2D[0]), float(bbox2D[1]), float(width_2d), float(height_2d)]
                labelSize = cv2.getTextSize(cat_id, cv2.FONT_HERSHEY_COMPLEX, 0.5, 4)
                # show : class
                _x1 = int(ct_x) + 10
                _y1 = int(ct_y) + 10
                ...
                cv2.putText(image, cat_id,
                            (int(ct_x) + 10, int(ct_y) + 10),
                ...
                ann = {'image_id': int(image_id),
                       ...
                       # 'depth': location_changed[2],  #tilt일때
                       'depth': location_cam_untilted[2],  # untilt일때
                       'alpha': alpha,
                       # 'truncated': truncated,
                       # 'occluded': occluded,
                       'location': location_cam_untilted,
                       'rotation_y': rotation_y
                       }
                #
                ...
                cv2.imshow("ori_inp" + str(image_id), image)
                # cv2.imshow("trans_inp" + str(image_id), inp)
                # cv2.imshow("resize_inp" + str(image_id), dst_in)
                cv2.imshow("trans_out" + str(image_id), out)
                # cv2.imshow("resize_out" + str(image_id), dst_out)
                cv2.waitKey()
                cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_untilted_04\\'+str(image_id)+'.jpg',image)
                cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_untilted_04_i\\'+str(image_id)+'.jpg', inp)
                cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_untilted_04_o\\' + str(image_id) + '.jpg', out)
            else:
                print("3d_ct out")
                print(image_id)
                print(cat_id)
