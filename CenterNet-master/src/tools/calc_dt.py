...
ann_dir = DATA_PATH + 'labelIndexing_3d_3_test/'
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        if os.path.isfile(ann_path):
            ...
            for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
                ...
                # dim_changed = [dim[i] for i in sun_permutation]  # 왜냐하면 dimension은 크기니까
...
for key in cat_dim_dict:
    ...
    print(key + "  dim_x\n" + "max {} min {} mean {} std {}".format(str(cat_max_x), str(cat_min_x), str(cat_mean_x),
                                                                    str(cat_std_x)))
