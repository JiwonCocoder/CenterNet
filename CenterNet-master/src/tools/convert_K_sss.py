...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            P = calib
            loc_2d_depth = unproject_2d_to_3d(pt_2d2, depth, P)
            loc_2d_depth_world = [loc_2d_depth[i] for i in sun_permutation]
            loc_2d_depth_world[2] *= -1
            box_3d_3 = compute_box_3d_world(dim, loc_2d_depth_world, rotation_y, theta)
            box_2d_3 = project_to_image(box_3d_3, calib)
            image_2 = draw_box_3d_world(image, box_2d_3, image_id, c=(255, 255, 0))
            box_3d = compute_box_3d_Rtilt(dim_changed, loc_2d_depth, rotation_y, Rtilt)
            box_2d = project_to_image(box_3d, calib)  # 이제 3d bounding box를 image에 투영시킴
            image2 = draw_box_3d_world(image, box_2d, image_id, c=(255, 255, 255))
            ...
            ann = {'image_id': int(image_id),
                   ...
                   'bbox': bbox2D_projected, #bbox는 xmin, ymin, width, height순
            ...
            cv2.imshow(str(image_id), image2)
            cv2.imshow(str(image_id), image5)
            cv2.waitKey()
