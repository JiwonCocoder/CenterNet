...
for split in splits:
    ...
    for image_id in f_list:
        ...
        for txt_3d in pd_list:
            ...
            # img = draw_box_3d(img, box_2d_pd, c=(0, 255, 0))
            # cv2.imshow(str(image_id), img)
            # cv2.waitKey()
        ...
        for key in pd_dict_per_img:
            ...
            if len(key_val_pd_list)> 0:
                ...
                for pd in pdKey_gtIOU_dict:
                    ...
                    #bbox2D_center
                    center_2d_pd_x = (bbox2D_pd_eval[0] + bbox2D_pd_eval[2])/2
                    center_2d_pd_y = (bbox2D_pd_eval[1] + bbox2D_pd_eval[3]) / 2
                    ...
                    print("genrerating pt_3d_world")
                    print(pt_3d_world)


                    box_3d_pd = compute_box_tilt_pd3(dim_w_pd_eval, pt_3d_world, yaw_pd_eval, Rtilt)
                    box_2d_pd = project_to_image(box_3d_pd, calib)
                    draw_box_3d(img, box_2d_pd, (0, 255, 255))
                    ...
                    # print(bbox2D_gt_eval)
            cv2.imwrite(
                'C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_tilt_hm_d\\' + str(
                    image_id) + '.jpg', img)
