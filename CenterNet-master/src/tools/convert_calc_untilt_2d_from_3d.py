...
#dim(world), loc(cam)
def compute_box_untilt_pd2(dim, location, rotation_y):
    ...
    corners_3d[:, 1] *= -1
    # location_cam
    corners_3d = np.transpose(corners_3d) + np.array(location, dtype=np.float32).reshape(3, 1)  # (3,8)
...
def read_pd_label(pd_label):
    ...
    box3d_pd = compute_box_untilt_pd2(dim_pd_w, loc_pd_c, rot_pd_w)
...
pd_dir = DATA_PATH + 'label_pd_2d_from_3d_untilt_last/'
...
for split in splits:
    ...
    for image_id in f_list:
        ...
        for txt_3d in pd_list:
            ...
            cat_pd, pd_label_list, box_3d_pd = read_pd_label(tmp_pd)
            box_2d_pd = project_to_image(box_3d_pd, calib)
            # img = draw_box_3d(img, box_2d_pd, c=(255, 0, 0))
            ...
            # cv2.line(img, (int(pd_label_list[3][0]), int(pd_label_list[3][1] )), (int(pd_label_list[3][2] ), int(pd_label_list[3][1] )), (0, 255, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(img, (int(pd_label_list[3][2] ), int(pd_label_list[3][1] )), (int(pd_label_list[3][2] ), int(pd_label_list[3][3] )), (0, 255, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(img, (int(pd_label_list[3][0]), int(pd_label_list[3][3] )), (int(pd_label_list[3][0]), int(pd_label_list[3][1] )), (0, 255, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(img, (int(pd_label_list[3][2] ), int(pd_label_list[3][3] )), (int(pd_label_list[3][0]), int(pd_label_list[3][3] )), (0, 255, 255), 2, lineType=cv2.LINE_AA)
