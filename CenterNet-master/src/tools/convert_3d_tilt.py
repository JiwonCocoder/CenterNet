...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]),int(ct3d_to_ct2d[0][1])), (int(end_x), int(end_y)), (0,255,0), 2) #초록
            alpha_x = ct3d_to_ct2d[0][0] + np.cos(alpha)*100
            alpha_y = ct3d_to_ct2d[0][1] - np.sin(alpha)*100
            cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]), int(ct3d_to_ct2d[0][1])), (int(alpha_x), int(alpha_y)), (255,0,0), 2) #파랑
