...
for split in splits:
    ...
    for image_id in f_list:
        ...
        for txt in anns_list:
            ...
            #     continue
            yaw_gt =gt_label_list[2] * 180 / math.pi
            yaw_gt_s = str(gt_label_list[2] * 180 / math.pi)[:4]
            if(yaw_gt < 0):
                yaw_changed = yaw_gt + 90
                rad_changed = yaw_changed * math.pi / 180
                print(rad_changed)
            ...
            if cat_id == 'table' or cat_id == 'desk':
                ...
                labelSize = cv2.getTextSize(yaw_gt_s, cv2.FONT_HERSHEY_COMPLEX, 0.5, 4)
                # show : class
                ...
                # cv2.putText(img, yaw_gt,
                #             (int(rot_end_x) + 5, int(rot_end_y) + 20),
                #             cv2.FONT_HERSHEY_COMPLEX, 0.7,
                #             (0,0,0), 2)
        ...
        for txt_3d in pd_list:
            ...
            if cat_pd == 'table' or cat_pd == 'desk':
                ...
                rot_end_x_pd = loc_2d[0][0] + np.cos(pd_label_list[2]) * 90
                rot_end_y_pd = loc_2d[0][1] - np.sin(pd_label_list[2]) * 90
                cv2.arrowedLine(img, (int(loc_2d[0][0]), int(loc_2d[0][1])),
                ...
                # show : class
                _x1 = int(rot_end_x_pd) + 5
                _y1 = int(rot_end_y_pd ) -3
                _x2 = _x1 + labelSize[0][0] + 15
                _y2 = _y1 + labelSize[0][1] + 10
                # cv2.rectangle(img , (_x1, _y1), (_x2, _y2), (255, 255, 255), cv2.FILLED)

                # cv2.putText(img, yaw_pd,
                #             (int(rot_end_x_pd) + 5, int(rot_end_y_pd) + 15),
                #             cv2.FONT_HERSHEY_COMPLEX, 0.7,
                #             cl, 2)
                # cv2.putText(img, yaw_gt,
                ...
                #             (int(loc_2d_pd[0][0]), int(loc_2d_pd[0][1]) - 10),
                #             cv2.FONT_HERSHEY_COMPLEX, 0.7,
                #             cl, 2)
