...
def compute_box_3d_world(dim, location, theta_c, theta):
  ...
  corners_3d[:, 1] *= -1
  print("------------------ground truth 3d bounding box coordinate--------------")
  print(corners_3d)
  return corners_3d
def compute_box_3d_world_tilt(dim, location, theta_c, theta):
  # print("roatation_y:" +str(rotation_y))
  c, s = np.cos(theta_c), np.sin(theta_c)
  print("rotation_y:" + str(theta_c) + "각도는" + str(theta_c * 180 / math.pi))
...
def compute_box_3d_world_untilt(dim, location, theta_c):
    # print("roatation_y:" +str(rotation_y))
    c, s = np.cos(theta_c), np.sin(theta_c)
    print("rotation_y:" + str(theta_c) + "각도는" + str(theta_c * 180 / math.pi))
...
# splits=['train']
# splits = ['train_Aug']
splits = ['allDataIndex_Aug_remake']
# splits = ['new_2']
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            location_compute = [float(tmp[1]), float(tmp[2]), float(tmp[3])]
            depth = location[1]
            #dim_w
            ...
            if Rtilt_flag :
                ...
                bbox2D = [float(tmp[8]), float(tmp[9]), float(tmp[8]) + float(tmp[10]), float(tmp[9]) + float(tmp[11])]# (0-based) bounding box of the object: Left, top, right, bottom image coordinates
            else:
            ...
            dim_yxz = [dim[i]*2 for i in [1,0,2]]
            dim_zxy = [dim[i]*2 for i in [2,0,1]]
            print("dim")
            print(dim)
            ...
            if Rtilt_flag:
                box_3d = compute_box_3d_world_tilt(dim_xyz, location_compute, rotation_y, theta)
            else:
                box_3d = compute_box_3d_world_untilt(dim_xyz, location, rotation_y)
            ...
            # json에 추가시키기.
            if( 0<ct3d_to_ct2d[0][0] < width -5 ) and (0<ct3d_to_ct2d[0][1] < height -5):

                # if(image[ct3d_to_ct2d[0][1]][ct3d_to_ct2d[0][0]][0]==0 and image[ct3d_to_ct2d[0][1]][ct3d_to_ct2d[0][0]][1]==0 and image[ct3d_to_ct2d[0][1]][ct3d_to_ct2d[0][0]][2]==0):
                # print(image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 0))
                # print(image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 1))
                # print(image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 2))
                if (image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 0) == 0 and
                        image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 1) == 0 and
                        image.item(ct3d_to_ct2d[0][1], ct3d_to_ct2d[0][0], 2) == 0):
                ...
                if alpha < -np.pi:
                    alpha += 2 * np.pi
                label_list = [cat_id,
                              location_compute[0], location_compute[1], location_compute[2],
                              dim[0], dim[1], dim[2],
                              rotation_y,
                ...
                cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]), int(ct3d_to_ct2d[0][1])), (int(alpha_x), int(alpha_y)),
                                (255, 0, 0), 2)

                end_x = ct3d_to_ct2d[0][0] + np.cos(rotation_y) * 100
                end_y = ct3d_to_ct2d[0][1] - np.sin(rotation_y) * 100
                cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]), int(ct3d_to_ct2d[0][1])), (int(end_x), int(end_y)),
                                (0, 255, 0), 2)
                ...
                ann = {'image_id': int(image_id),
                       ...
                       'depth': depth,  # untilt일때
                       'alpha': alpha,
                       # 'truncated': truncated,
                       # 'occluded': occluded,
                       #'location': location_cam_untilted,
                       'rotation_y': rotation_y,
                       'ct_2d_from_3d': ct3d_proj
                       }
                #
            ...
            # cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_Aug\\'+str(image_id)+'.jpg',image)
        ...
        if len(label_lists) == 0:
            continue
        fp = open('../../data/SUNRGBD/label_Aug2/'+'{}.txt'.format(image_id), 'w')
        ...
        # cv2.imwrite(
        #     'C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\img_ImsizeRatio\\' + str(
        #         image_index) + '.jpg', img)
    # 이미지 보이기
    # if DEBUG:
