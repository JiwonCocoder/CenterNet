...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            xmin = int(bbox2D[0])
            ymin = int(bbox2D[1])
            xmax = int(bbox2D[0] + bbox2D[2])
            ymax = int(bbox2D[1] + bbox2D[3])
            x = (bbox2D[0] + bbox2D[2]) / 2

            print(cat_id, location, dim, rotation_y, bbox2D)
            #ann에 axis 바꾼거를 집어넣기
            ...
            ann = {'image_id': int(image_id),
                 ...
                 #  'category_name' : cat_id,
                   ...
                   'depth': location[2],
                   'alpha': rot_y2alpha(-rotation_y, x, Rtilt[0,2], Rtilt[0,0]),
                  # 'truncated': truncated,
                  # 'occluded': occluded,
            ...
            if DEBUG:
                ...
                # calib[2][3] = 0.005
                #img 보이기
                box_3d = compute_box_3d_sun_6(dim, location, rotation_y, Rtilt)
