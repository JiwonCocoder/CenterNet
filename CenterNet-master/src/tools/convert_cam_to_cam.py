...
from utils.ddd_utils import draw_box_3d_world,draw_box_3d, unproject_2d_to_3d
...
def compute_box_3d(dim_changed, location_changed, rotation_y):
    ...
    # z_corners = [z/2, -z/2, -z/2, z/2, z/2, -z/2, -z/2, z/2]
    #
    corners = np.array([x_corners, y_corners, z_corners], dtype=np.float32) #(3,8)
    ...
    corners_3d = corners_3d + np.array(location_changed, dtype=np.float32).reshape(3, 1)  #(3,8)
