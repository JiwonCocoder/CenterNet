  ...
  for ind_f in range(3, -1, -1):
    ...
    for j in range(4):
      ...
      cv2.line(image, (int(corners[f[j], 0]), int(corners[f[j], 1])),
               (int(corners[f[(j+1)%4], 0]), int(corners[f[(j+1)%4], 1])), c, 5, lineType=cv2.LINE_AA)
      if ind_f == 2:  # 암면에 대해서는 대각선으로 표시
        cv2.line(image, (int(corners[f[0], 0]), int(corners[f[0], 1])),
                 (int(corners[f[2], 0]), int(corners[f[2], 1])), c, 2, lineType=cv2.LINE_AA)
        cv2.line(image, (int(corners[f[1], 0]), int(corners[f[1], 1])),
                 (int(corners[f[3], 0]), int(corners[f[3], 1])), c, 2, lineType=cv2.LINE_AA)
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            cv2.putText(image, label, (xmin_3d_to_2d + 10, ymin_3d_to_2d + 10), cv2.FONT_HERSHEY_COMPLEX, 0.5,
                        (0, 0, 0), 1)
            ...
            cv2.waitKey()
            # cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_ct3d_3\\'+str(image_id)+'.jpg',image)
