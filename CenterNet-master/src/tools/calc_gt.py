...
def compute_box_3d(dim_changed, location_changed, rotation_y):
    ...
    corners_3d = corners_3d + np.array(location_changed, dtype=np.float32).reshape(3,
                                                                                   1)  # camera좌표 (0,0)에서 시작했었으니까, 물체 center point로 평행이동시킴
...
ann_dir = DATA_PATH + 'labelIndexing_ct3d/'
...
# splits = ['train_3d_2', 'val_3d_2', 'test_3d_2']
# splits = ['testDataIndex']
# splits=['train', 'val']
# splits = ['allDataIndexNew']
splits = ['allDataIndex_3d_2']
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            print("Rtilted location_changed")
            ...
            print(dim_changed)
            #
            # cat_dep = cat_dep_dict[cat_id]
            # cat_dep.append(location_changed[2])
            # cat_dep_dict[cat_id] = cat_dep

            cat_dim = cat_dim_dict[cat_id]
            cat_dim.append(dim_changed)
            cat_dim_dict[cat_id] = cat_dim
            ...
            ct3d_to_ct2d_ori = project_to_image(np.array([location_ori_changed]),
                                                calib)  # 이제 3d bounding box를 image에 투영시킴
            print(ct3d_to_ct2d)
            # 빨간색 점(3d center의 2d화)에 대한 2d box
            cv2.circle(image, (int(ct3d_to_ct2d[0][0]), int(ct3d_to_ct2d[0][1])), 5, (0, 0, 255), -1)

            xmin_3d_to_2d = int(ct3d_to_ct2d[0][0] - width_2d / 2)
            ymin_3d_to_2d = int(ct3d_to_ct2d[0][1] - height_2d / 2)
            xmax_3d_to_2d = int(ct3d_to_ct2d[0][0] + width_2d / 2)
            ymax_3d_to_2d = int(ct3d_to_ct2d[0][1] + height_2d / 2)
            ...
            # if ymin_3d_to_2d < 0:
            #     ymin_projected = 0
            # if xmax_3d_to_2d >= width:
            #     xmax_projected = width - 1
            # if ymax_3d_to_2d >= height:
            ...
            # cv2.line(image, (xmin_3d_to_2d, ymin_3d_to_2d), (xmax_3d_to_2d, ymin_3d_to_2d), (0, 0, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (xmax_3d_to_2d, ymin_3d_to_2d), (xmax_3d_to_2d, ymax_3d_to_2d), (0, 0, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (xmin_3d_to_2d, ymax_3d_to_2d), (xmin_3d_to_2d, ymin_3d_to_2d), (0, 0, 255), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (xmax_3d_to_2d, ymax_3d_to_2d), (xmin_3d_to_2d, ymax_3d_to_2d), (0, 0, 255), 2, lineType=cv2.LINE_AA)
            bbox3D_to_2D = [float(xmin_3d_to_2d), float(ymin_3d_to_2d), float(width_2d),
                            float(height_2d)]

            # label 도 붙여보기
            label = cat_id
            labelSize = cv2.getTextSize(label, cv2.FONT_HERSHEY_COMPLEX, 0.5, 2)
            _x1 = xmin_3d_to_2d + 10
            _y1 = ymin_3d_to_2d + 10
            _x2 = _x1 + labelSize[0][0]
            _y2 = _y1 - labelSize[0][1]


print("ground truth dimension statistics")
...
for key in cat_dim_dict:
    val = cat_dim_dict[key] # nx3

    # val_xyz = val.transpose() #3xn
    dim_x = np.array(val)[:, 0]
    dim_y = np.array(val)[:, 1]
    dim_z = np.array(val)[:, 2]
    #
    cat_max_x, cat_min_x, cat_mean_x, cat_std_x = calc_stats(dim_x)
    cat_max_y, cat_min_y, cat_mean_y, cat_std_y = calc_stats(dim_y)
    cat_max_z, cat_min_z, cat_mean_z, cat_std_z = calc_stats(dim_z)

    print(key + "  dim_x\n" + "max {} min {} mean {} std {}".format(str(cat_max_x), str(cat_min_x), str(cat_mean_x), str(cat_std_x)))
    print(key + "  dim_y\n" + "max {} min {} mean {} std {}".format(str(cat_max_y), str(cat_min_y), str(cat_mean_y),
                                                                    str(cat_std_y)))
    print(key + "  dim_z\n" + "max {} min {} mean {} std {}".format(str(cat_max_z), str(cat_min_z), str(cat_mean_z),
                                                                    str(cat_std_z)))

print("ground truth depth statistics")
# for key in cat_dep_dict:
#     val = cat_dep_dict[key] # nx3
#     dep = np.array(val)
#     cat_max, cat_min, cat_mean, cat_std = calc_stats(dep)
#
#     print(key + "  dep\n" + "max {} min {} mean {} std {}".format(str(cat_max), str(cat_min), str(cat_mean), str(cat_std)))
#     print(key + len(dep))