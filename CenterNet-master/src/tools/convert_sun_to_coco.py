...
SPLITS = ['3dop', 'subcnn']
import _init_paths
from utils.ddd_utils import compute_box_3d, project_to_image, alpha2rot_y, compute_box_3d_sun, compute_box_3d_sun_2
    ...
    for split in splits:
        ...
        for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
            ...
            for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
                ...
                location = [float(tmp[1]), float(tmp[3]), float(tmp[2])]
                ...
                dim = [float(tmp[4]), float(tmp[6]), float(tmp[5])]  # 3D object dimensions: height, width, length [m]
                ...
                if DEBUG and tmp[0] != 'DontCare':
                    ...
                    # calib[1][3] = -0.34
                    # calib[2][3] = 0.005
                    box_3d = compute_box_3d_sun_2(dim, location, rotation_y)
