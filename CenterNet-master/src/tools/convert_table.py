...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            theta = rotation_y * 180 / math.pi
            print("for original theta, radian:{}, {}".format(str(theta), str(rotation_y)))
            # rotation_y = 0
            # if theta > 0 :
            #     theta -= 180
            # elif theta < 0:
            #     theta += 180
            # rotation_y = theta * math.pi / 180
            print("for error theta, radian:{}, {}".format(str(theta), str(rotation_y)))
            ...
            # Rtilt =np.array([[1,0,0],[0,1,0], [0,0,1]])
