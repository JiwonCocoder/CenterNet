...
def read_gt_label(gt_label, Rtilt):
    ...
    return cat_id, [loc_gt_w_tilt, dim_gt_w, rot_gt_w, bbox2D_gt] , box_3d
...
def read_pd_label(pd_label, Rtilt):
    ...
    bbox2D_pd = [float(pd_label[8]), float(pd_label[9]), float(pd_label[10]), float(pd_label[11])]
    box3d_pd = compute_box_tilt_pd3(dim_pd_w, loc_pd_w, rot_pd_w, Rtilt)

    # Rotation around Y-axis in camera coords. [-Pi; Pi]
    return cat_id, [loc_pd_w, dim_pd_w, rot_pd_w, bbox2D_pd] , box3d_pd
...
ann_dir = DATA_PATH + 'label_Aug2/'
Rtilt_dir = DATA_PATH + 'Rtilt_Aug/'
calib_dir = DATA_PATH + 'calib_Aug/'
img_dir = DATA_PATH + "img_Aug/"
pd_dir_Aug = DATA_PATH + 'label_pd_not_Aug/'
# pd_dir_Aug = DATA_PATH + 'label_pd_Aug_except/'
pd_dir_not_Aug = DATA_PATH + 'label_pd_not_Aug/'
...
for split in splits:
    ...
    error_mean_dict_per_cat = {"bathtub": [], "bed": [], "bookshelf": [],
                       "chair": [], "desk": [], "dresser": [], "nightstand": [],
                       "sofa": [], "table": [], "toilet": []}
    error_dict_per_toilet_not_Aug = {"image_id": [], "gt": [], "error": []}
    error_dict_per_toilet_Aug = {"image_id": [], "gt": [], "pd": [], "error": []}
    image_id_list = []
    error_toilet_list = []
    for image_id in f_list:
        ...
        # print(image_id)
        # image_id = 10763
        # img_path = img_dir + '{}.jpg'.format(str(int(image_id) - 10335))
        # calib_path = calib_dir + '{}.txt'.format(str(int(image_id) - 10335))
        # Rtilt_path = Rtilt_dir + '{}.txt'.format(str(int(image_id) - 10335))
        # ann_path = ann_dir + '{}.txt'.format(str(int(image_id) - 10335))

        img_path = img_dir + '{}.jpg'.format(str(int(image_id)))
        calib_path = calib_dir + '{}.txt'.format(str(int(image_id)))
        Rtilt_path = Rtilt_dir + '{}.txt'.format(str(int(image_id)))
        ann_path = ann_dir + '{}.txt'.format(str(int(image_id)))
        ...
        if len(pd_lines_Aug) == 0:
            print("empty gt file")
            continue
        pd_list_Aug = removeWhiteSpaceInfile(pd_lines_Aug)

        # generate pd_dict_per_img
        for txt_3d in pd_list_Aug:
            tmp_pd = txt_3d[1:-1].split(',')
            cat_pd, pd_label_list, box_3d_pd = read_pd_label(tmp_pd, Rtilt)
            if cat_pd == 'bed':
                image_id_list.append(image_id)

            box_2d_pd = project_to_image(box_3d_pd, calib)
            cl = colors_jw[ID[cat_pd]]

            img = draw_box_3d(img, box_2d_pd, c=cl)
            #
            # if pd_label_list[3][0] < 0 :
            #     pd_label_list[3][0]  = 0
            # if pd_label_list[3][1]  < 0 :
            #     pd_label_list[3][1] = 0
            # if pd_label_list[3][2] > img_w -1:
            #     pd_label_list[3][2] = img_w - 1
            # if pd_label_list[3][3] > img_h:
            #     pd_label_list[3][3]= img_h -1
            # cv2.imshow(str(image_id), img)
            # cv2.waitKey()
        ...
        for key in pd_dict_per_img:
            ...
            if len(key_val_pd_list)> 0:
                ...
                for pd in pdKey_gtIOU_dict:
                    ...
                    print(bbox2D_gt_eval)
                    draw_2d_box(img2, bbox2D_gt_eval[0], bbox2D_gt_eval[1], bbox2D_gt_eval[2], bbox2D_gt_eval[3] , c=(128, 128, 128))
                    draw_2d_box(img2, bbox2D_pd_eval[0], bbox2D_pd_eval[1], bbox2D_pd_eval[2], bbox2D_pd_eval[3], c=(0, 255, 0))
                    # cv2.imshow("img", img2)
                    # cv2.waitKey()
                    ...
                    if(key=="sofa"):
                        error_toilet_list.append([image_id, np.round(loc_w_gt_eval[0],4), [error_loc_w, error_dim_w, error_yaw]])
                        error_dict_per_toilet_Aug["image_id"].append(image_id)
                        error_dict_per_toilet_Aug["gt"].append([loc_w_gt_eval.tolist(), dim_w_gt_eval.tolist(), yaw_gt_eval])
                        error_dict_per_toilet_Aug["pd"].append([loc_w_pd_eval.tolist(), dim_w_pd_eval.tolist(), yaw_pd_eval])
                        error_dict_per_toilet_Aug["error"].append([error_loc_w, error_dim_w, error_yaw])
                    # error_yaw = yaw_pd_eval - yaw_gt_eval
    ...
    for key_error in error_dict_per_cat:
        ...
        #     continue
        print(len(error_dict_per_cat[key_error]))
        error_list = error_dict_per_cat.get(key_error)
        error_list_len = len(error_list)
        error_array = np.array(error_list)

        total_error_loc_x.append(error_array[:, 0].tolist())
        total_error_loc_y.append(error_array[:, 1].tolist())
        total_error_loc_z.append(error_array[:, 2].tolist())
        total_error_dim_x.append(error_array[:, 3].tolist())
        total_error_dim_y.append(error_array[:, 4].tolist())
        total_error_dim_z.append(error_array[:, 5].tolist())

        #
        # for i in range(6):
        #     temp_total_error_list.append(error_array[:, i].tolist())
        # print(temp_total_error_list)
        error_array[:,6] = np.rint((error_array[:,  6]* 180/ math.pi))
        total_error_yaw.append(error_array[:, 6].tolist())
    ...
    temp =np.array(sum(total_error_loc_x, []))
    total_error.append(np.mean(temp))
    # temp1 = np.hstack()
    # temp = np.array(total_error_loc_x).reshape(-1)

    temp =np.array(sum(total_error_loc_y, []))
    total_error.append(np.mean(temp))

    temp = np.array(sum(total_error_loc_z, []))
    total_error.append(np.mean(temp))

    temp = np.array(sum(total_error_dim_x, []))
    total_error.append(np.mean(temp))

    temp = np.array(sum(total_error_dim_y, []))
    total_error.append(np.mean(temp))

    temp = np.array(sum(total_error_dim_z, []))
    total_error.append(np.mean(temp))

    temp = np.array(sum(total_error_yaw, []))
    total_error.append(np.mean(temp))

    print(np.round(total_error,4))
    image_id_list= error_dict_per_toilet_Aug.get("image_id")
    gt_list = error_dict_per_toilet_Aug.get("gt")
    pd_list = error_dict_per_toilet_Aug.get("pd")
    error_list = error_dict_per_toilet_Aug.get("error")
    for i in range(len(error_dict_per_toilet_Aug["image_id"])):
        print('image_id:{}, gt:{}, pd:{}, error:{}'.format(image_id_list[i], gt_list[i], pd_list[i], error_list[i]))
    print("error_toilet")
    print(error_toilet_list)
    # total_error_array = np.array(total_error)
