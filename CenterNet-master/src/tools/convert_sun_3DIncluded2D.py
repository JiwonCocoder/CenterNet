...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            box_3d = compute_box_3d(dim_changed, location_changed, rotation_y, image_id)
            # print("before neg")
            # print(rotation_y)
            # rotation_y = -rotation_y
            # print("after neg")
            ...
            # cv2.line(image, (int(xmin_projected), int(ymin_projected)), int((xmax_projected), int(ymin_projected)), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (int(xmax_projected), int(ymin_projected)), int((xmax_projected), int(ymax_projected)), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (int(xmin_projected), int(ymax_projected)), int((xmin_projected), int(ymin_projected)), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            # cv2.line(image, (int(xmax_projected), int(ymax_projected)), int((xmin_projected), int(ymax_projected)), (0, 255, 0), 2, lineType=cv2.LINE_AA)
            ...
            ann = {'image_id': int(image_id),
                   ...
                   'dim': dim_changed,
                   'bbox': bbox2D_projected, #'bbox': box2D
            ...
            image = draw_box_3d_sun(image, box_2d, image_id)
            cv2.imshow(str(image_id), image)
            cv2.waitKey()
            # cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_3Dincluded2D\\'+str(image_id)+'.jpg',image)
    print(changed_RotIndex_list)
    print(changed_RotIndex_list.sort())
    ...
    #out_path = '{}/annotations/sun_{}.json'.format(DATA_PATH+'200112', split)
    # out_path = 'C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\sun_{}.json'.format(split)
