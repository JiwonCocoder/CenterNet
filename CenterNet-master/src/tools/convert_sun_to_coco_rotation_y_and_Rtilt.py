...
SPLITS = ['3dop', 'subcnn']
import _init_paths
from utils.ddd_utils import compute_box_3d, project_to_image, project_to_image_sun2, project_to_image_sun, alpha2rot_y, \
    compute_box_3d_sun, compute_box_3d_sun_2, compute_box_3d_sun_3, compute_box_3d_sun_4, compute_box_3d_sun_5, \
    compute_box_3d_sun_6,compute_box_3d_sun_8
...
ann_dir = DATA_PATH + 'labelIndexing_notRtilted(191220)/'
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            location = [float(tmp[1]), float(tmp[2]), float(tmp[3])]  ##z는 이미 - 되어져서 온 상황 from label.txt에서
            ...
            cv2.circle(image, (int(ct_x), int(ct_y)), 3, (0, 255, 0), -1)
            end_x = ct_x + np.cos(rotation_y)*100
            end_y = ct_y + np.sin(rotation_y)*100
            cv2.arrowedLine(image, (int(ct_x),int(ct_y)), (int(end_x), int(end_y)), (0,255,0), 2)
            #alpha = rot_y2alpha(rotation_y, x, calib[0, 2], calib[0, 0])
            alpha_x = ct_x + np.cos(alpha)*100
            alpha_y = ct_y + np.sin(alpha)*100
            cv2.arrowedLine(image, (int(ct_x), int(ct_y)), (int(alpha_x), int(alpha_y)), (255,0,0), 2)
            ...
            dim_changed = [dim[i] for i in sun_permutation]
            ann = {'image_id': int(image_id),
                   ...
                   'bbox': _bbox_to_coco_bbox(bbox2D),
                   #'depth': abs(location_changed[1]), ....NotRtilted(2019-12-23)
                   'depth' : location_changed[1],
                   'alpha': alpha,
                   # 'truncated': truncated,
                   # 'occluded': occluded,
