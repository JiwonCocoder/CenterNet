...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            cv2.circle(image, (int(ct_x), int(ct_y)), 5, (0, 255, 0), -1)
            height_2d = ymax - ymin
            width_2d = xmax - xmin
            ...
            ct3d_to_ct2d_ori = project_to_image(np.array([location_ori_changed]), calib)  # 이제 3d bounding box를 image에 투영시킴
            print(ct3d_to_ct2d)
            ...
            alpha = rotation_y - np.arctan2(xmin_3d_to_2d - calib[0][2], calib[0][0])
            if alpha > np.pi:
                alpha -= 2 * np.pi
            if alpha < -np.pi:
                alpha += 2 * np.pi
            ...
            _x1 = int(ct3d_to_ct2d[0][0]) + 10
            _y1 = int(ct3d_to_ct2d[0][1]) + 10
            _x2 = _x1 + labelSize[0][0]
            _y2 = _y1 - labelSize[0][1]

            #show : tilt
            # label_tilt = str("x:{} y:{} z:{}".format(str(round(theta[0],2)), str(round(theta[1],2)), str(round(theta[2],2))))
            # labelSize_tilt = cv2.getTextSize(label_tilt, cv2.FONT_HERSHEY_TRIPLEX, 3, 5)
            # _x3 = _x1 -10
            # _y3 = _y1 + 20
            # _x4 = _x3 + 200
            # _y4 = _y3 - 10

            # cv2.circle(image, (int(ct3d_to_ct2d_ori[0][0]), int(ct3d_to_ct2d_ori[0][1])), 10, (255, 255, 255), -1)
            ...
            pt_2d = [ct_x_projected, ct_y_projected]
            ...
            box_3d = compute_box_3d_Rtilt(dim_changed, loc_2d_depth, rotation_y, Rtilt)
            box_2d = project_to_image(box_3d, calib)  # 이제 3d bounding box를 image에 투영시킴
            ...
            #글자와 box의 선명도를 위해서
            image = draw_box_3d_world(image, box_2d, image_id)
            cv2.rectangle(image, (_x1, _y1), (_x2, _y2), (0, 0, 255), cv2.FILLED)
            cv2.putText(image, label, (int(ct3d_to_ct2d[0][0]) + 10, int(ct3d_to_ct2d[0][1]) + 10),
                        cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 1)
            #show tilt
            # cv2.rectangle(image, (_x3, _y3), (_x4, _y4), (0, 0, 255), cv2.FILLED)
            # cv2.putText(image, label_tilt, (_x3, _y3),
            #             cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 255), 1)
            # cv2.putText(image, label_tilt, (int(ct3d_to_ct2d[0][0]) -20, int(ct3d_to_ct2d[0][1])+20),
            #             cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 255), 1)
            ...
            #world로 넣어줬던 상황
            # location_changed2 = [location_changed[i] for i in sun_permutation]
            # location_changed2[2] *= -1
            ...
            # cv2.imshow(str(image_id), image_2d)

            # cv2.waitKey()
            cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\img_2d_vs_3d\\'+str(image_id)+'.jpg',image)
