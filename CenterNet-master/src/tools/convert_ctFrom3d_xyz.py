...
def draw_box_2d(image, xmin, ymin, xmax, ymax):
    cv2.line(image, (int(xmin), int(ymin)), (int(xmax), int(ymin)), (255, 255, 255), 2, lineType=cv2.LINE_AA)
    cv2.line(image, (int(xmax), int(ymin)), (int(xmax), int(ymax)), (255, 255, 255), 2, lineType=cv2.LINE_AA)
    cv2.line(image, (int(xmin), int(ymax)), (int(xmin), int(ymin)), (255, 255, 255), 2, lineType=cv2.LINE_AA)
    cv2.line(image, (int(xmax), int(ymax)), (int(xmin), int(ymax)), (255, 255, 255), 2, lineType=cv2.LINE_AA)
    ct_x = (xmin + xmax) / 2
    ct_y = (ymin + ymax) / 2
    cv2.circle(image, (int(ct_x), int(ct_y)), 10, (255, 255, 255), -1)

def draw_box_2d_from_ct(image, ct, width, height):
    xmin = ct[0] - width/2
    ymin = ct[1] - height/2
    xmax = ct[0] + width/2
    ymax = ct[1] + height/2
    print("draw_box_2d_from_ct")
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            #화면 밖 bbox고려시 (ct_x, ct_y, width_projected, height_projected)
            ...
            #화면에 맞게 bbox clip시 (ct_x, ct_y, width, height)
            ...
            width_projected = xmax_projected - xmin_projected
            height_projected = ymax_projected - ymin_projected
            print("ground_truth width, height (projected")
            print(width_projected, height_projected)
            ...
            #(ct3d_to_ct2d 와 ct_projected)사이의 거리와 (ct3d_to_ct2d 와 ct) 사이의 거리를 비교한 후 가까운 쪽 bbox 그리기
            distance_projected1 = cal_distance(ct3d_to_ct2d[0], np.array([ct_x_projected, ct_y_projected]))
            distance_projected2 = cal_distance(ct3d_to_ct2d[0], np.array([ct_x, ct_y]))
            if distance_projected1 < distance_projected2:
                print("yellow")
                # draw_box_2d(image, xmin_projected, ymin_projected, xmax_projected, ymax_projected)
                draw_box_2d_from_ct(image, ct3d_to_ct2d[0], width_projected, height_projected)
                bbox2D_projected = [float(ct3d_to_ct2d[0][0]), float(ct3d_to_ct2d[0][1]), float(width_projected),
                                    float(height_projected)]

            else:
                # draw_box_2d(image, xmin, ymin, xmax, ymax)
                draw_box_2d_from_ct(image, ct3d_to_ct2d[0], width_2d, height_2d)
                bbox2D_projected = [float(ct3d_to_ct2d[0][0]), float(ct3d_to_ct2d[0][1]), float(width_2d),
                                    float(height_2d)]
            #19.12.26 추가된 부분
            print("Rtilt")
            print(Rtilt)
            ...
            # i = open('./train.txt', mode = 'at', encoding = 'utf-8')
            #print(box_2d.transpose()[0].min(), box_2d.transpose()[0].max(), box_2d.transpose()[1].min(), box_2d.transpose()[1].max())
            if(xmin_projected >= 0 and ymin_projected >= 0 and xmax_projected < width -1 and ymax_projected < height -1):
    ...
    print(img_used)
    # file = open('./val.txt', 'w')
    # img_used_list = list(img_used)
    # for i in range(len(img_used_list)):
    #     file.write(img_used_list[i] + '\n')
    # file.close()
