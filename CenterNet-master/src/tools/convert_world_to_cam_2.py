...
def compute_box_3d_world(dim, location, theta_c, theta):
  ...
  if(-abs(c) > - abs(s)):
      new_theta = math.atan(c/-s)
      theta_c = new_theta
      c = np.cos(new_theta)
      s = np.sin(new_theta)
      print("new_rotation_y:" + str(new_theta) + "각도는" + str(new_theta * 180 / math.pi))
...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            box_3d, rotation_y_new = compute_box_3d_world(dim, location, rotation_y, theta)
            ...
            end_x = int(ct3d_to_ct2d[0][0]) + np.cos(rotation_y)*100
            end_y = int(ct3d_to_ct2d[0][1]) - np.sin(rotation_y)*100

            end_x_new = int(ct3d_to_ct2d[0][0]) + np.cos(rotation_y_new) * 100
            end_y_new = int(ct3d_to_ct2d[0][1]) - np.sin(rotation_y_new) * 100

            cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]),int(ct3d_to_ct2d[0][1])), (int(end_x), int(end_y)), (255,255,255), 2) #초록
            cv2.arrowedLine(image, (int(ct3d_to_ct2d[0][0]), int(ct3d_to_ct2d[0][1])), (int(end_x_new), int(end_y_new)),
                            (0, 255, 0), 2)  # 초록
