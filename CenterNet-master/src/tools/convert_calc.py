...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            # json에 추가시키기.
            if (ct3d_to_ct2d[0][0] > width - 1) or (ct3d_to_ct2d[0][1] > height - 1):
                continue
            class_count_index = int(cat_ids[cat_id])-1
            class_count[class_count_index][0] = class_count[class_count_index][0] + 1
            ...
            # bbox_y
            cv2.line(image, (int(xmin_projected), int(ymin_projected)), (int(xmax_projected), int(ymin_projected)),
                     (0, 255, 255), 2, lineType=cv2.LINE_AA)
            cv2.line(image, (int(xmax_projected), int(ymin_projected)), (int(xmax_projected), int(ymax_projected)),
                     (0, 255, 255), 2, lineType=cv2.LINE_AA)
            cv2.line(image, (int(xmin_projected), int(ymax_projected)), (int(xmin_projected), int(ymin_projected)),
                     (0, 255, 255), 2, lineType=cv2.LINE_AA)
            cv2.line(image, (int(xmax_projected), int(ymax_projected)), (int(xmin_projected), int(ymax_projected)),
                     (0, 255, 255), 2, lineType=cv2.LINE_AA)
            ...
            # cv2.waitKey()
            # cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_untilted_03_new\\'+str(image_id)+'.jpg',image)
