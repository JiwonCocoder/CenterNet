...
for split in splits:
    ...
    for line in image_set:  # train.txt에 있는 line을 str로 하나씩 읽어들임
        ...
        for ann_ind, txt in enumerate(anns):  # 한줄씩 처리한다는 의미: ann_ind는 object 갯수에 대한 index.
            ...
            bbox3DFlag = 1
            if ct3d_to_ct2d[0][0] < 0:
                bbox3DFlag = 0
            if ct3d_to_ct2d[0][1] < 0:
                bbox3DFlag = 0
            if ct3d_to_ct2d[0][0] >= width:
                bbox3DFlag = 0
            if ct3d_to_ct2d[0][1] >= height:
                bbox3DFlag = 0
            label_list = []
            if bbox3DFlag != 0 :
            ...
            xmin_projected_rot = np.cos(rotation_y)*width_projected
            xmax_projected_rot = np.cos(rotation_y)*width_projected
            ymin_projected_rot = np.sin(rotation_y)*height_projected
            ymax_projected_rot = np.sin(rotation_y) * height_projected
            ...
            # cv2.waitKey()
            # cv2.imwrite('C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\resultImages_ct3d\\'+str(image_id)+'.jpg',image)
        ...
        if len(label_lists) == 0:
            print("out_of_boundary_image_id" + str(image_id))
            continue
        # ct_path = 'C:\\obj_detection\\CenterNet-master\\CenterNet-master\\data\\SUNRGBD\\labelIndexing_ct3d\\'
        # fp = open(ct_path + '{}.txt'.format(image_id), 'w')
